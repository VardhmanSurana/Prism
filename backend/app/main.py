...
@app.get("/api/v1/sample-images/{filename}")
async def serve_sample_image(filename: str):
    """Serve sample/demo images from the sample_images directory."""
    import os
    from fastapi.responses import FileResponse, JSONResponse

    sample_dir = os.path.join(os.path.dirname(__file__), "..", "sample_images")
    file_path = os.path.normpath(os.path.join(sample_dir, filename))

    # ponytail: path traversal guard
    if not file_path.startswith(os.path.normpath(sample_dir)):
        return JSONResponse(status_code=403, content={"error": "Access denied"})

    if not os.path.isfile(file_path):
        return JSONResponse(status_code=404, content={"error": "Sample image not found"})
...
app.include_router(telemetry_api.router, prefix=f"{settings.API_V1_STR}", tags=["telemetry"], dependencies=[Depends(verify_api_key)])
