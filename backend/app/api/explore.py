...
async def explore_photos(
    ...
    """Root explore/search endpoint combining metadata + semantic search."""
    filters = _base_photo_filters()
    ...
    if q:
        ...
        filters.append(
            or_(
                ...
                Photo.caption.ilike(search_pattern),
                Photo.city.ilike(search_pattern),
                Photo.state.ilike(search_pattern),
                Photo.country.ilike(search_pattern),
                Photo.ai_summary.ilike(search_pattern),
                Photo.ocr_text.ilike(search_pattern),
            )
        )
    ...
    return {
        "results": [photo_to_dict(p) for p in photos],
        "total": len(photos),
        "query": q,
    }
