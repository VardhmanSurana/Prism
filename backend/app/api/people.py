...
# ── Photo face scan (Rust-only, Python stub for parity) ─────────────────────


@router.post("/scan/{photo_id}")
async def scan_photo_faces(
    photo_id: int,
    db: AsyncSession = Depends(get_db),
):
    """POST /api/v1/people/scan/{photo_id} — Scan faces in a specific photo.

    In the Rust backend this delegates to the Python ML service.
    In the Python backend, face scanning is handled by the background
    processing queue (``processing_queue`` + ``vision_pipeline``).

    This endpoint is a TODO stub that mirrors the Rust API signature.
    """
    photo = await db.get(Photo, photo_id)
    if not photo:
        raise HTTPException(status_code=404, detail="Photo not found")

    # TODO: Implement real on-demand face scan via InsightFace SDK.
    # The background pipeline already handles this — this endpoint is for
    # explicit re-scans on demand.
    return {
        "photo_id": photo_id,
        "faces_detected": 0,
        "status": "todo",
        "message": "On-demand face scan not yet implemented in Python backend. "
                   "Face scanning is handled by the background processing queue.",
    }
