...
async def export_photos_zip(
    payload: ExportPhotosRequest,
    db: AsyncSession = Depends(get_db),
):
    """Export multiple photos as a zip archive."""
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        for photo in photos:
            try:
                ...
                logger.warning(f"Failed to add {photo.filename} to zip: {e}")

    zip_buffer.seek(0)
    return StreamingResponse(
        zip_buffer,
        media_type="application/zip",
        headers={"Content-Disposition": f'attachment; filename="prism_export_{len(photos)}_photos.zip"'},
    )
