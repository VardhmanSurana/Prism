...
async def lock_photo(photo_id: str, db: AsyncSession = Depends(get_db)):
    ...
    await db.commit()
    return {"status": "success", "message": f"Photo {photo.id} locked and encrypted."}


@router.post("/{photo_id}/unlock")
async def unlock_photo(photo_id: str, db: AsyncSession = Depends(get_db)):
    if not locked_service.is_authenticated:
        raise HTTPException(status_code=403, detail="Locked Folder session not authenticated")
    ...
    await db.commit()
    return {"status": "success", "message": f"Photo {photo.id} unlocked and decrypted."}
