...
PEOPLE_COLUMN_PATCHES: tuple[tuple[str, str], ...] = (
    ("uuid", "VARCHAR(36)"),
)
...
async def apply_schema(conn: AsyncConnection) -> None:
    ...
    try:
        await _add_missing_columns(conn, "people", PEOPLE_COLUMN_PATCHES)
    except Exception as e:
        logger.debug("people column patch skipped: %s", e)
    ...
    await _backfill_uuids(conn)

    await _bump_schema_version(conn, CURRENT_SCHEMA_VERSION)
    logger.info("Schema patches applied successfully.")


async def _backfill_uuids(conn: AsyncConnection) -> None:
    """Assign UUIDs to rows created before the uuid columns existed."""
    import uuid as _uuid

    for table in ("photos", "albums", "people", "video_projects"):
        res = await conn.execute(text(f"SELECT id FROM {table} WHERE uuid IS NULL OR uuid = ''"))
        for (row_id,) in res.fetchall():
            await conn.execute(
                text(f"UPDATE {table} SET uuid = :u WHERE id = :id"),
                {"u": str(_uuid.uuid4()), "id": row_id},
            )
        if res.rowcount:
            logger.info("Backfilled UUIDs for %s (%d rows)", table, res.rowcount)
