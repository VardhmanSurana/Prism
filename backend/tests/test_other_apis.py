...
@pytest.mark.asyncio
async def test_telemetry_settings_api(tmp_path, monkeypatch):
    from app.config import settings
    monkeypatch.setattr(settings, "SETTINGS_FILE", tmp_path / "settings.json")

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        get_res = await client.get("/api/v1/settings/telemetry")
        assert get_res.status_code == 200
        assert get_res.json() == {"enabled": True, "sample_rate": 10, "response_logging": False}

        post_res = await client.post("/api/v1/settings/telemetry", json={
            "enabled": False, "sample_rate": 5000, "response_logging": True
        })
        assert post_res.status_code == 200
        assert post_res.json()["enabled"] is False
        assert post_res.json()["sample_rate"] == 1000
        assert post_res.json()["response_logging"] is True

        get_res = await client.get("/api/v1/settings/telemetry")
        assert get_res.json() == {"enabled": False, "sample_rate": 1000, "response_logging": True}

        partial_res = await client.post("/api/v1/settings/telemetry", json={"sample_rate": 1})
        assert partial_res.json()["enabled"] is False
        assert partial_res.json()["sample_rate"] == 1
