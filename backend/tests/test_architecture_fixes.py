...
async def test_schema_migration_versioning():
    ...
    async with engine.begin() as conn:
        ...
        # Initial run: schema_version table starts at version 0, then bumps to current
        ...
        assert v_after == CURRENT_SCHEMA_VERSION

        # Second run: idempotent, version stays unchanged
        await apply_schema(conn)
        v_second = await _ensure_schema_version(conn)
        assert v_second == CURRENT_SCHEMA_VERSION
