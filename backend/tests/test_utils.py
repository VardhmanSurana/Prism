...
def test_security_resolve_read_write():
    # Full filesystem access: arbitrary system paths resolve for read and write
    assert safe_resolve_read("/etc/passwd") == Path("/etc/passwd").resolve()
    assert safe_resolve_write("/etc/passwd") == Path("/etc/passwd").resolve()
