...
@pytest.mark.asyncio
async def test_list_dir_rejects_non_directory():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        # A file path (even outside the library dirs) is a valid full-access path,
        # but listing it should fail cleanly since it is not a directory.
        response = await client.post(
            "/api/v1/utilities/list-dir",
