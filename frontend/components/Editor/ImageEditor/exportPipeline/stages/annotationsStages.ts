...
import { smoothPath, getAnnotationBBox, getSvgRotationTransform } from '../../AnnotationCanvas/utils';
import {
  getPolygonPoints,
  getShapePathString,
  normalizeBounds,
  ...
  return new Promise((resolve) => {
    ...
    annotations.forEach(ann => {
      ...
      if (ann.type === 'freehand' && ann.points) {
        ...
        const rotTransform = getSvgRotationTransform(rotVal, cx, cy, aspectRatio);
        const rotAttr = rotTransform ? ` transform="${rotTransform}"` : '';
        ...
        const cy = y + b.h / 2;
        const rotTransform = getSvgRotationTransform(rotVal, cx, cy, aspectRatio);
        const transformAttr = rotTransform ? ` transform="${rotTransform}"` : '';
        ...
        if (baseBgColor || ann.bgGlass) {
          const bgTransform = rotTransform ? ` transform="${rotTransform}"` : '';
        ...
        subSvg += `<text x="${textX}" y="${textY}" text-anchor="${textAnchor}" font-size="${fontSize}" fill="${ann.color}"${transformAttr} style="${textStyle}">`;
